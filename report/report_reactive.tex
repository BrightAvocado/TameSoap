...
The second experiment investigates the performance difference between RLA trained agents and dummy agents. Dummy agents were simulated using the code provided in \texttt{ReactiveTemplate.java}. We used the two dummy agents specified in \texttt{agents.xml}: \texttt{reactive-random-85}, and \texttt{reactive-random-50}. We compared these dummy agents to an RLA trained agent using a discount factor of 0.99, specified by \texttt{reactive-rla-99}.\\

We ran each simulation for the four different topologies provided (the Netherlands, England, France and Switzerland).\\

\subsubsection{Observations}
% elaborate on the observed results
We can see that the RLA trained agent performs consistently better than both dummy agents across all four topologies. This shows that the RLA trained agent makes "wiser" decisions.\\
